 ...
 *
 * encryptionKey removed: E2EE is now handled automatically via ECDH key pair
 * generated by MessageManager on construction. Use getPublicKey() to access
 * this instance's public key for distribution to peers.
 */
...
export class ColaLinkManager extends EventEmitter<ColaLinkEvents> {
  ...
  constructor(config: ColaLinkConfig) {
    ...
    // Initialize managers (encryptionKey no longer needed: MessageManager
    // generates its own ECDH key pair for E2EE)
    this.contactManager = new ContactManager(this.dbPath);
    this.messageManager = new MessageManager(this.dbPath);
    ...
    logger.info(`My E2EE public key: ${this.messageManager.getPublicKey()}`);
    this.emit('ready');
  }
  ...
  /**
   * Get this instance's E2EE public key for distribution to peers
   */
  getPublicKey(): string {
    return this.messageManager.getPublicKey();
  }

  /**
   * Set a peer's public key for E2EE message encryption
   *
   * Must be called before sending messages to a contact.
   * This is typically done during contact request/accept flow.
   */
  setPeerPublicKey(handle: string, publicKey: string): void {
    this.messageManager.setPeerPublicKey(handle, publicKey);
  }
  ...
  async sendMessage(toHandle: string, content: string): Promise<Message> {
    ...
    // Ensure peer public key is stored
    const existingKey = this.messageManager.getPeerPublicKey(toHandle);
    if (!existingKey && contact.publicKey && contact.publicKey !== 'pending') {
      // Auto-set from contact's stored publicKey
      this.messageManager.setPeerPublicKey(toHandle, contact.publicKey);
    }
    ...
    if (this.wechatPlugin && this.wechatPlugin.isWeChatHandle(toHandle)) {
      try {
        ...
        logger.error(
          `Failed to send to WeChat: ${message.id}`,
          error as Error
        );
  ...
  addContact(
    ...
    // Auto-register peer public key if provided
    if (contact.publicKey && contact.publicKey !== 'pending') {
      this.messageManager.setPeerPublicKey(contact.handle, contact.publicKey);
    }
  ...
  sendContactRequest(
    ...
    return this.contactManager.sendRequest(
      this.config.myHandle,
      toHandle,
      message
    );
  ...
  acceptContactRequest(requestId: string): ContactRequest {
    ...
    if (!existing) {
      this.contactManager.addContact({
        ...
        publicKey: 'pending', // Should be exchanged during request flow
        status: 'friend',
      });
    }
  ...
  > {
    return this.messageManager.getRecentConversations(
      this.config.myHandle,
      limit
    );
  }
